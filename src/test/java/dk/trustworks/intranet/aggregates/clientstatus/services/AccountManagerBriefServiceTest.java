...
class AccountManagerBriefServiceTest {
    ...
    @Test
    void buildPayload_includesGapsProjectsConsultantsAndExcluded() {
        var consultant = new ConsultantGap("Sara Vest", 316_000, 0, 316_000);
        var project = new ProjectGap("Project Management", 185_119);
        var gap = new GapMonth("januar 2026", 1_856_127, 1_135_147, 720_980,
                List.of(consultant), List.of(project));
        var over = new OverMonth("marts 2026", 96_000);
        var client = new ClientAnalysis("Banedanmark", List.of(gap), List.of(over));

        ObjectNode payload = AccountManagerBriefService.buildPayload(
                "Tommy", Framing.TO_AM, List.of(client), List.of("VATTENFALL VINDKRAFT A/S"));
        ...
        JsonNode m0 = c0.get("gapMonths").get(0);
        assertEquals("januar 2026", m0.get("month").asText());
        assertEquals(720_980, m0.get("missing").asLong());
        assertEquals("Sara Vest", m0.get("consultantsMissing").get(0).get("name").asText());
        assertEquals(185_119, m0.get("projectGaps").get(0).get("missing").asLong());
        assertEquals("marts 2026", c0.get("overMonths").get(0).get("month").asText());
        assertEquals(96_000, c0.get("overMonths").get(0).get("surplus").asLong());
    ...
    void buildPayload_selfFraming_encodedInPayload() {
        ObjectNode payload = AccountManagerBriefService.buildPayload(
                "Tommy", Framing.SELF, List.of(), List.of());
    ...
    void buildPayload_sanitizesAccountManagerName() {
        ObjectNode payload = AccountManagerBriefService.buildPayload(
                "<script>Tommy</script>", Framing.TO_AM, List.of(), List.of());
        assertEquals("Tommy", payload.get("accountManager").asText(),
