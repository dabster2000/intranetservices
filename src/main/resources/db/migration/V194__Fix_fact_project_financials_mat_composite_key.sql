-- =============================================================================
-- Migration V194: Fix fact_project_financials_mat composite key
--
-- Problem:
--   The project_financial_id in V118 is CONCAT(project_uuid, '-', month_key)
--   but the view grain includes companyuuid. When a project-month spans
--   multiple companies, INSERT IGNORE in sp_refresh_fact_tables silently
--   drops duplicate rows, causing ~25% revenue undercount.
--
-- Fix:
--   1. Update V118 view to include companyuuid in project_financial_id
--   2. Widen the PK column to accommodate the longer ID
--   3. Update sp_refresh_fact_tables to use REPLACE instead of INSERT IGNORE
--   4. Re-refresh the materialized table
-- =============================================================================

-- Step 1: Recreate the view with companyuuid in the project_financial_id
...
-- 10) Service line: dominant primary skill per project-month
...
SELECT
    -- FIX: Include companyuuid in the composite key to prevent INSERT IGNORE from dropping multi-company rows
...
-- Step 2: Widen the PK column and update the materialized table
ALTER TABLE fact_project_financials_mat
    MODIFY COLUMN project_financial_id VARCHAR(120) NOT NULL;

-- Step 3: Update sp_refresh_fact_tables to use REPLACE instead of INSERT IGNORE
-- for fact_project_financials_mat (prevents silent row drops on duplicate keys)
...
BEGIN
    ...
    -- 3. fact_project_financials_mat (FIXED: TRUNCATE + INSERT to avoid INSERT IGNORE issue)
    TRUNCATE TABLE fact_project_financials_mat;
    INSERT INTO fact_project_financials_mat
...
-- Step 4: Re-refresh to populate with correct data
