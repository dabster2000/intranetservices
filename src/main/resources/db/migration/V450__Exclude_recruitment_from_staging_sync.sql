-- ====================================================================
-- V450: Keep ALL recruitment / ATS data out of the prod->staging sync.
--
-- WHY: The recruitment_* and candidate_dossier* tables hold live candidate
-- PII (names, contact details, CVs/cover letters, consent records, interview
-- notes, AI-extracted content). Recruitment is GDPR-governed: lawful-basis
-- tracking, retention deadlines, Art. 14 notices and auto-anonymization at
-- deadline (see docs/superpowers/plans/2026-07-18-recruitment-ats-expansion.md).
-- Phase 2 of this procedure does NOT anonymize any recruitment column, so the
-- generic Phase 1 copy would land raw production candidate PII in staging every
-- night. Excluding these tables from the sync means staging keeps its own
-- recruitment test data and no production candidate PII is ever copied over.
--
-- MAINTENANCE (explicit list, on purpose): the recruitment block below must be
-- EXTENDED whenever a new recruitment_* / candidate_* base table is added by a
-- future migration. A table not listed here is silently copied from prod again,
-- re-introducing candidate PII into staging. Keep this list in sync with the
-- ATS schema. (See the paired memory note on this migration.)
--
-- Body is V406 verbatim with only the cur_tables NOT IN (...) list extended by
-- the recruitment block. No other change.
-- Rollback: re-apply V406's procedure body.
-- ====================================================================
