-- Migration V118: Fix companyuuid propagation for expenses and prevent margin distortion when filtering by company
--
-- Summary:
-- - Distributes project expenses across companies at the project-month grain using revenue share when available,
--   otherwise work hours share; falls back to main company when neither is available.
-- - Keeps companyuuid consistent across all cost components to align revenue and cost at the same dimensional grain.
--
-- Rationale:
-- Previous version assigned all project expenses to a single hardcoded companyuuid, which distorted margins when
-- filtering by company (revenue could belong to company A while expenses were booked to company B). This migration
-- fixes that by allocating expenses proportionally per company per project-month.
--
-- Note: This migration intentionally recreates the view. It is idempotent via CREATE OR REPLACE VIEW.
...
-- 1) Project-month base from work entries (ensures we only consider operational months)
project_months AS (
    SELECT DISTINCT
...
--    Prefer revenue share if there is revenue in the month; otherwise use work hours share; otherwise default to main company
revenue_weights AS (
    SELECT
        rbc.project_uuid,
        rbc.year_val,
...
-- 9) Total cost per project-month-company (employee + external + distributed expenses)
total_cost_aggregation AS (
    ...
    -- c) rows where we only have expenses (no work or external)
    SELECT
        pebc.project_uuid,
        pebc.year_val,
        pebc.month_val,
...
-- 10) Service line: dominant primary skill per project-month (no company split)
...
dominant_service_line AS (
    SELECT slr.project_uuid, slr.year_val, slr.month_val, slr.primaryskilltype AS dominant_skilltype
    FROM service_line_ranking slr
    WHERE slr.skill_rank = 1
),

-- 11) Contract type per project (no company split)
...
project_month_companies AS (
    ...
    -- months with work but no revenue or cost: assign to main company
...
SELECT
    CONCAT(pmc.project_uuid, '-', CONCAT(LPAD(pmc.year_val, 4, '0'), LPAD(pmc.month_val, 2, '0'))) AS project_financial_id,
