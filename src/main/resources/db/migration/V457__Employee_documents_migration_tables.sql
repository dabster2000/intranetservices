-- ===================================================================
-- V457: Employee Documents Phase 2a — migration working tables
-- ===================================================================
-- Feature: Employee Documents S3-only storage, Increment 2a
--          (spec docs/superpowers/specs/2026-07-24-employee-documents-s3-only-design.md
--           §6.2 V45c + §9; runbook 2026-07-25-employee-documents-phase2-runbook.md
--           step 2a-1, AI-assist decisions A1–A5)
-- Domain:  documentservice (SharePoint→S3 migration tooling; the tables
--          are TEMPORARY and are dropped again at decommission, spec §9.8)
--
-- Purpose:
--   1. sharepoint_migration_folders — one row per top-level personal
--      folder per SharePoint site: the matching problem ("whose folder
--      is this?"). Carries the AI match proposal columns (decisions
--      A1/A2 — proposals are durable and human-confirmable, never
--      auto-applied) and the AI_CONFIRMED match method.
--   2. sharepoint_migration_items — one row per file: the resumability
--      problem ("which exact files exist, and has each one been copied
--      and verified?"). Unique on drive_item_id so re-runs skip work.
--   3. Seed the migration AI kill switch
--      employee_documents.migration.ai.enabled = 'false' (decision A5;
--      removed again with the tooling, spec §9.8).
--   4. Re-create sp_sync_prod_to_staging with BOTH new tables added to
--      the exclusion list — V453's header demands this happens in the
--      same migration wave (a table not listed is silently copied from
--      prod every night). Body is V453 verbatim + the two-table
--      extension + one owed fix: the mail anonymisation block now also
--      scrubs the V455 columns reply_to / cc / bcc (recruiter and
--      candidate addresses; parked until the V453 procedure change-set
--      had merged — both change-sets must live in one body).
--
-- Idempotency: repair-at-start re-runs migrations across checkouts —
--   CREATE TABLE IF NOT EXISTS, INSERT IGNORE for the settings seed,
--   DROP PROCEDURE IF EXISTS + CREATE for the procedure.
--
-- Collation: utf8mb4_general_ci (schema default); UUIDs VARCHAR(36).
--
-- Author: Claude Code
-- Date:   2026-07-27
-- Rollback: tooling is dark (admin-triggered only) —
--     DROP TABLE IF EXISTS sharepoint_migration_items;
--     DROP TABLE IF EXISTS sharepoint_migration_folders;
--     DELETE FROM app_settings WHERE setting_key = 'employee_documents.migration.ai.enabled';
--     re-apply V453's procedure body.
-- ===================================================================

-- -------------------------------------------------------------------
-- 1. Folders — the matching problem (spec §6.2 V45c + A1/A2 columns)
-- -------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS sharepoint_migration_folders (
    id                BIGINT AUTO_INCREMENT PRIMARY KEY,
    site_url          VARCHAR(500)  NOT NULL,
    folder_path       VARCHAR(1024) NOT NULL COMMENT 'Top-level personal folder path relative to the drive root',
    folder_name       VARCHAR(500)  NOT NULL,
    matched_user_uuid VARCHAR(36)   NULL,
    match_method      ENUM('USERNAME','FULLNAME','AI_CONFIRMED','MANUAL','UNMATCHED')
                      NOT NULL DEFAULT 'UNMATCHED',
    -- AI proposals (decision A2): durable + human-confirmable. A proposal
    -- lives here until an admin confirms it (match_method=AI_CONFIRMED)
    -- or picks someone else (MANUAL). Never auto-applied.
    ai_suggested_user_uuid VARCHAR(36) NULL,
    ai_confidence     ENUM('HIGH','MEDIUM','LOW') NULL,
    ai_reason         VARCHAR(512)  NULL COMMENT 'One-sentence justification shown in the admin card',
    file_count        INT           NOT NULL DEFAULT 0,
    total_bytes       BIGINT        NOT NULL DEFAULT 0,
    status            ENUM('DISCOVERED','MAPPED','COPYING','VERIFIED','FAILED','SKIPPED')
                      NOT NULL DEFAULT 'DISCOVERED',
    note              VARCHAR(1024) NULL,
    -- Prefix lengths keep the composite key under InnoDB's 3072-byte cap
    -- (utf8mb4); site URLs and top-level folder paths are far shorter.
    UNIQUE KEY uq_smf (site_url(255), folder_path(255))
);

-- -------------------------------------------------------------------
-- 2. Items — the resumability problem (spec §6.2 V45c)
-- -------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS sharepoint_migration_items (
    id              BIGINT AUTO_INCREMENT PRIMARY KEY,
    folder_id       BIGINT        NOT NULL,
    drive_item_id   VARCHAR(255)  NOT NULL,
    relative_path   VARCHAR(1024) NOT NULL COMMENT 'Path under the personal folder (empty for root files)',
    name            VARCHAR(500)  NOT NULL,
    size_bytes      BIGINT        NOT NULL,
    quickxor_hash   VARCHAR(64)   NULL COMMENT 'From Graph; compared against a local recompute at copy time',
    etag            VARCHAR(128)  NULL,
    status          ENUM('DISCOVERED','COPIED','VERIFIED','FAILED','SKIPPED')
                    NOT NULL DEFAULT 'DISCOVERED',
    employee_document_uuid VARCHAR(36) NULL,
    error           VARCHAR(1024) NULL,
    UNIQUE KEY uq_smi (drive_item_id),
    KEY idx_smi_folder (folder_id),
    KEY idx_smi_status (status)
);

-- -------------------------------------------------------------------
-- 3. AI kill switch (decision A5) — OFF by default; the migration
--    stays fully functional deterministically with this off.
-- -------------------------------------------------------------------
INSERT IGNORE INTO app_settings (setting_key, setting_value, category)
VALUES ('employee_documents.migration.ai.enabled', 'false', 'employee_documents');

-- -------------------------------------------------------------------
-- 4. Extend the prod→staging sync exclusion list (V453 body + the two
--    migration working tables) and fold in the owed mail-copy scrub
--    (reply_to / cc / bcc from V455).
-- -------------------------------------------------------------------
