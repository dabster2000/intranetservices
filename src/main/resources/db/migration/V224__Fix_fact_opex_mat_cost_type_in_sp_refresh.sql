-- =============================================================================
-- Migration V224: Fix fact_opex_mat cost_type in sp_refresh_fact_tables
--
-- Root cause:
--   V221 re-created sp_refresh_fact_tables() to optimize blocks 5 and 11 but
--   accidentally dropped `cost_type` from block 4 (fact_opex_mat) INSERT/SELECT.
--   MariaDB used DEFAULT 'OTHER' for all rows, breaking CXO dashboard routes
--   that filter on cost_type IN ('OPEX','SALARIES').
--
-- Fix:
--   1. Re-create sp_refresh_fact_tables() — identical to V221 except block 4
--      now includes `cost_type` in both the INSERT column list and SELECT.
--   2. Immediately re-materialize fact_opex_mat from fact_opex to restore
--      correct cost_type values without waiting for the nightly refresh.
--
...
--   V221: Optimized blocks 5 and 11 — accidentally dropped cost_type from block 4
--   V224: Restores cost_type in block 4 (this migration)
--
...
BEGIN
    ...
    -- 4. fact_opex_mat (FIXED: cost_type restored — was missing in V221)
...
-- ---------------------------------------------------------------------------
-- Immediate re-materialization of fact_opex_mat
--
-- Repopulates from the fact_opex view so the mat table contains correct
-- cost_type values ('OPEX' and 'SALARIES') immediately without waiting for
-- the nightly sp_nightly_bi_refresh() run.
--
-- NOTE: We do NOT call sp_refresh_fact_tables() here because it refreshes
-- all 11 tables and would block Flyway for too long. Only fact_opex_mat
-- needs refreshing — all other mat tables are unaffected by this fix.
-- ---------------------------------------------------------------------------
TRUNCATE TABLE fact_opex_mat;
