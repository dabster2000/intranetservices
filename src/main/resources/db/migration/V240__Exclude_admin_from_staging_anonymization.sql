-- =============================================================================
-- Migration V240: Improve staging sync procedure
--
-- Changes from V234:
--   1. Exclude admin user (hans.lassen) from PII anonymization so Azure AD
--      login works in staging
--   2. Neutralize expense statuses to prevent e-conomics uploads in staging
--   3. Exclude integration_keys table from sync (contains staging-specific
--      e-conomics API tokens and journal numbers)
--
-- Rollback:
--   Re-run V234 procedure creation
-- =============================================================================
...
BEGIN
    ...
    -- Cursor: all base tables except flyway_schema_history and integration_keys
    -- (integration_keys contains staging-specific e-conomics API tokens)
    DECLARE cur_tables CURSOR FOR
        SELECT TABLE_NAME
        FROM INFORMATION_SCHEMA.TABLES
        WHERE TABLE_SCHEMA = 'twservices4'
          AND TABLE_TYPE = 'BASE TABLE'
          AND TABLE_NAME NOT IN ('flyway_schema_history', 'integration_keys');
    ...
    -- VALIDATED/PROCESSING → CREATED (upload job won't pick them up)
