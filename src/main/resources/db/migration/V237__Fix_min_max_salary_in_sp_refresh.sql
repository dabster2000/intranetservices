-- =============================================================================
-- Migration V237: Fix min/max salary columns in sp_refresh_fact_tables
--
-- Root cause:
--   V222 correctly added MIN(s.salary) and MAX(s.salary) to the avg_salary CTE
--   and the INSERT column list for fact_minimum_viable_rate_mat. V224 then
--   recreated sp_refresh_fact_tables() based on the pre-V222 version, which
--   overwrote the min/max salary additions. The columns exist in the table
--   (added by V222's ALTER TABLE) but the refresh SP never populates them.
--
-- Fix:
--   1. Re-create sp_refresh_fact_tables() — identical to V224 except block 11
--      now includes min_monthly_salary / max_monthly_salary in:
--      - the avg_salary CTE (MIN/MAX aggregation)
--      - the INSERT column list
--      - the final SELECT
--   2. Call sp_refresh_fact_tables() once to populate the NULL columns.
--
...
--   V224: Restored cost_type in block 4 — accidentally dropped min/max salary from block 11
--   V237: Restores min/max salary in block 11 (this migration)
--
...
-- ---------------------------------------------------------------------------
-- Immediately refresh all fact tables to populate the NULL min/max salary
-- columns. This runs the full SP so all mat tables are consistent.
-- ---------------------------------------------------------------------------
