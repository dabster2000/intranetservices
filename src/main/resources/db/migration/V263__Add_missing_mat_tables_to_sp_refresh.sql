-- =============================================================================
-- Migration V263: Add missing mat tables to sp_refresh_fact_tables
--
-- Root cause:
--   fact_company_revenue_mat and fact_internal_invoice_cost_mat were originally
--   added to sp_refresh_fact_tables in V202 and V196 respectively. However,
--   subsequent migrations (V221, V220) that recreated the procedure accidentally
--   dropped these blocks. All later recreations (V224, V237) perpetuated the
--   omission, leaving these two materialized tables permanently stale.
--
-- Impact:
--   fact_company_revenue_mat is the primary revenue source for the CXO Dashboard
--   Profitability (FY) chart. Without refresh, the chart shows stale revenue data.
--   Example: Feb 2026 shows 3.0M DKK instead of the correct 7.1M DKK — a 57%
--   understatement that makes EBITDA appear ~4.1M worse than reality.
--
--   fact_internal_invoice_cost_mat is used for intercompany cost tracking and
--   source data export. Also stale by ~2M DKK for recent months.
--
-- Fix:
--   Re-create sp_refresh_fact_tables() with all 13 mat table blocks:
--   - Blocks 1-11: unchanged from V237
--   - Block 12: fact_company_revenue_mat (restored from V202)
--   - Block 13: fact_internal_invoice_cost_mat (restored from V196)
--
...
--   V220: Added 5 more mat tables (blocks 7-11) — accidentally dropped V196 and V202 blocks
--   V221: Optimized blocks 5 and 11 — perpetuated the V202 drop
--   V222: Added min/max salary columns to block 11
--   V224: Restored cost_type in block 4 — dropped min/max salary from block 11
--   V237: Restores min/max salary in block 11
--   V263: Restores fact_company_revenue_mat and fact_internal_invoice_cost_mat (this migration)
--
-- Idempotent: DROP PROCEDURE IF EXISTS + CREATE — safe to re-run.
-- =============================================================================
...
BEGIN
    ...
    -- =========================================================================
    -- 12. fact_company_revenue_mat  (RESTORED — dropped accidentally in V221)
    --
    -- Source view: fact_company_revenue (V201)
    -- Grain: company_id × month_key
    -- Used by: CXO Dashboard Profitability chart (EBITDA revenue component)
    -- =========================================================================
    ...
    -- =========================================================================
    -- 13. fact_internal_invoice_cost_mat  (RESTORED — dropped accidentally in V220)
    --
    -- Source view: fact_internal_invoice_cost (V196)
    -- Grain: company_id × month_key
    -- Used by: CXO Dashboard EBITDA source data export, intercompany cost tracking
    -- =========================================================================
...
-- ---------------------------------------------------------------------------
-- Immediately refresh all fact tables to populate fact_company_revenue_mat
-- and fact_internal_invoice_cost_mat. This runs the full SP so all 13 mat
