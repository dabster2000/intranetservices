-- V275: Simplify utilization to read from fact_user_day + add post-first-billing column
--
-- 1. Align with V272 convention: all utilization reads from fact_user_day.
--    Replaces 3 complex CTEs (utilization_by_user_month from bi_data_per_day +
--    billable_by_user_month from work_full + utilization_stats join) with a
--    single aggregation from fact_user_day which already has pre-computed
--    registered_billable_hours and net_available_hours.
--
-- 2. Add utilization_post_first_billing column to fact_minimum_viable_rate_mat.
--    This excludes the ramp-up period before a consultant's first billable work,
--    giving a fairer utilization metric for the Career Level Cost Structure tab.
--    Other consumers (break-even dashboard) continue using actual_utilization_ratio.

ALTER TABLE fact_minimum_viable_rate_mat
    ADD COLUMN utilization_post_first_billing DECIMAL(10,4) DEFAULT NULL
    AFTER actual_utilization_ratio;
...
BEGIN
    ...
    WITH
        ...
        -- Replaces the previous 3-CTE approach (bi_data_per_day + work_full + join).
