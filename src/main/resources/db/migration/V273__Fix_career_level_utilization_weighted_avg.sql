-- V273: Fix career level utilization to use weighted aggregate instead of unweighted average
--
-- The utilization_stats CTE in sp_refresh_fact_tables was computing
-- AVG(billable/available per user per month), which gives a misleadingly low
-- utilization number because it weights each user-month equally regardless of
-- available hours.
--
-- Fix: Change to SUM(billable) / SUM(available) per career level per company,
-- which is the standard weighted utilization metric.
--
-- Impact: fact_minimum_viable_rate_mat.actual_utilization_ratio will increase
-- to reflect the true aggregate utilization per career level.
    ...
    WITH
        ...
        utilization_stats AS (
            SELECT
                ...
                CASE WHEN SUM(uum.net_available_hours) > 0
                     THEN SUM(COALESCE(bbm.billable_hours, 0)) / SUM(uum.net_available_hours)
                     ELSE 0
