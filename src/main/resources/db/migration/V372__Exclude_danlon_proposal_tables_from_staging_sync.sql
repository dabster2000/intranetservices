-- ====================================================================
-- V372: Exclude danlon_assignment_proposal + danlon_number_sequence
--       from the prod->staging sync.
--
-- These hold environment-specific app state: staging maintains its own
-- proposal queue and its own monotonic counter. Without exclusion the
-- nightly generic copy would overwrite staging's test proposals and
-- reset staging's counter to prod's value mid-test
-- (reference_staging_nightly_refresh_strips_unpromoted_migrations).
--
-- Body is V306 verbatim with the two table names added to the
-- cur_tables NOT IN (...) list. No other change.
-- Rollback: re-apply V306's procedure body.
-- ====================================================================
