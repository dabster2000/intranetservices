-- =============================================================================
-- Migration V223: Hours-weighted average billing rate
--
-- Purpose:
--   The avg_billing_rate in the actual_rates CTE was computed as a simple
--   AVG(wf.rate) — an unweighted average treating every work registration row
--   equally regardless of hours logged. A 1-hour registration at kr. 1500 and
--   a 40-hour registration at kr. 1000 contributed equally, producing misleading
--   results. This migration changes to an hours-weighted average:
--   SUM(rate * workduration) / NULLIF(SUM(workduration), 0).
--
-- Changes:
--   1. CREATE OR REPLACE VIEW fact_minimum_viable_rate — change AVG(wf.rate) to
--      hours-weighted formula in the actual_rates CTE.
--   2. Recreate sp_refresh_fact_tables — same change in the embedded actual_rates
--      CTE inside the stored procedure.
--
-- Backwards compatibility:
--   All columns and column names are unchanged. Only the computed values for
--   avg_actual_billing_rate (and derived rate_buffer_dkk, break_even_utilization_*)
--   will differ due to the weighting correction.
-- =============================================================================


-- ---------------------------------------------------------------------------
-- 1. CREATE OR REPLACE VIEW: hours-weighted avg_billing_rate
-- ---------------------------------------------------------------------------
...
WITH
    ...
    -- V223: Changed from AVG(wf.rate) to hours-weighted average
    actual_rates AS (
        SELECT
            wf.consultant_company_uuid AS company_id,
            ccl.career_level,
            SUM(wf.rate * wf.workduration) / NULLIF(SUM(wf.workduration), 0) AS avg_billing_rate
        FROM work_full wf
...
-- ---------------------------------------------------------------------------
-- 2. Recreate sp_refresh_fact_tables with hours-weighted avg_billing_rate
--    Base: V222 SP. Only the actual_rates CTE in block 11 is changed.
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    --     V223: changed AVG(wf.rate) to hours-weighted average in actual_rates
    -- =========================================================================
