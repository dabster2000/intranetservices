-- V272: Remove fact_user_utilization_mat table and fact_user_utilization view
--
-- All application code has been migrated to read from fact_user_day directly.
-- The materialized table and its source view are no longer needed.
--
-- Changes:
--   1. DROP TABLE fact_user_utilization_mat
--   2. DROP VIEW  fact_user_utilization
--   3. Recreate sp_refresh_fact_tables without Block 1 (the mat table refresh)
--      and with Block 11 updated to compute utilization from fact_user_day
--      instead of the dropped mat table.
--
-- Block renumbering: old 2→1, 3→2, 4→3, 5→4, 6→5, 7→6, 8→7, 9→8, 10→9, 11→10, 12→11, 13→12, 14→13

-- =========================================================================
-- Step 1: Drop the materialized table
-- =========================================================================
DROP TABLE IF EXISTS fact_user_utilization_mat;

-- =========================================================================
-- Step 2: Drop the source view
-- =========================================================================
DROP VIEW IF EXISTS fact_user_utilization;

-- =========================================================================
-- Step 3: Recreate sp_refresh_fact_tables without the utilization block
-- =========================================================================
...
BEGIN
    ...
    WITH
        ...
        -- V272: Replaced fact_user_utilization_mat with direct computation
        -- from fact_user_day (via bi_data_per_day alias) + work_full.
        -- This produces the same monthly net_available / billable / utilization
        -- values that the dropped mat table contained.
        utilization_by_user_month AS (
            SELECT
                b.companyuuid,
                ...
                SUM(COALESCE(b.net_available_hours, 0)) AS net_available_hours
            FROM bi_data_per_day b
            WHERE b.consultant_type = 'CONSULTANT'
              AND b.status_type <> 'TERMINATED'
            GROUP BY b.useruuid, b.companyuuid, b.year, b.month
        ),

        billable_by_user_month AS (
            SELECT
                wf.useruuid AS user_id,
                YEAR(wf.registered) AS year_val,
                MONTH(wf.registered) AS month_val,
                SUM(wf.workduration) AS billable_hours
            FROM work_full wf
            WHERE wf.registered IS NOT NULL
              AND wf.workduration > 0
              AND wf.rate > 0
              AND wf.type = 'CONSULTANT'
            GROUP BY wf.useruuid, YEAR(wf.registered), MONTH(wf.registered)
        ),

        utilization_stats AS (
            SELECT
                uum.companyuuid AS company_id,
                ccl.career_level,
                AVG(uum.net_available_hours) AS avg_net_available_hours,
                AVG(COALESCE(bbm.billable_hours, 0)) AS avg_billable_hours,
                AVG(CASE WHEN uum.net_available_hours > 0
                         THEN COALESCE(bbm.billable_hours, 0) / uum.net_available_hours
                         ELSE 0
                    END) AS avg_utilization_ratio
            FROM utilization_by_user_month uum
            LEFT JOIN billable_by_user_month bbm
                ON bbm.user_id = uum.user_id
                AND bbm.year_val = uum.year_val
                AND bbm.month_val = uum.month_val
            INNER JOIN user u ON u.uuid = uum.user_id
            INNER JOIN userstatus us ON us.useruuid = u.uuid
            INNER JOIN current_career_level ccl ON ccl.useruuid = u.uuid
            CROSS JOIN ttm_range tr
            WHERE CAST(uum.month_key AS CHAR CHARACTER SET utf8mb4) >= tr.start_month_key
              AND CAST(uum.month_key AS CHAR CHARACTER SET utf8mb4) <= tr.end_month_key
              AND us.type = 'CONSULTANT'
              AND u.type = 'USER'
              AND us.statusdate = (
                  SELECT MAX(us2.statusdate)
            ...
            GROUP BY uum.companyuuid, ccl.career_level
        ),
