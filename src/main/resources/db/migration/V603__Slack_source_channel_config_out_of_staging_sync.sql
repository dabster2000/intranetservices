-- ============================================================================
-- V603: keep the Slack source-channel list and its run log out of staging
-- ============================================================================
--
-- WHAT THIS IS
--
-- One change, to one list: sp_sync_prod_to_staging stops copying
-- crm_slack_source_channel and crm_slack_sync_run into staging. No table is
-- created, altered or dropped, and no row is written.
--
-- WHY
--
-- Staging's Slack bot token is byte-identical to production's. The two
-- environments do not read a copy of the same workspace; they read the same
-- workspace. V602 classed the channel list as configuration and let it copy,
-- following the rule the TrustLink alias tables set, and that rule is right
-- everywhere it does not collide with a shared credential. Here it does: the
-- night crm.slack.source-channels.enabled is switched on in production, the
-- sync hands staging both the flag and production's channel list, and staging
-- starts reading the same real channels, sending the same days to OpenAI, and
-- writing mentions nobody will ever open. Twice the spend and twice the reach
-- into a workspace full of people who were told this happens once.
--
-- app_settings is deliberately still copied. The flag is harmless to inherit
-- once the list is not: staging's job wakes, finds no enabled channel of its
-- own, logs one line and returns. That is also what makes a staging rehearsal
-- possible at all -- a test channel listed on staging now survives the night
-- instead of being replaced by production's list.
--
-- crm_slack_sync_run is the same argument in a smaller key. A run log is a
-- record of what one environment did. Copied, staging's Settings -> CRM tab
-- shows production's runs as its own history, and a production run still open
-- when the sync lands leaves staging's Run now button disabled behind a
-- tooltip describing a run that is not happening there.
--
-- WHAT IS STILL COPIED, ON PURPOSE
--
-- slack_unmatched_company, its sighting ledger and the sighting authors. They
-- are the same shape as calendar_unmatched_domain and calendar_unmatched_meeting,
-- which V601 copies with their per-person attribution, and they are useful in
-- staging: the hints panel has something to render without staging reading
-- anything itself. account_slack_mention and account_slack_mention_participant
-- stay excluded, as V602 set them.
--
-- HOW
--
-- MySQL cannot patch a stored procedure, so the whole body is re-emitted. It is
-- lifted VERBATIM from V602 -- the newest migration that defines it -- with the
-- two names added to the NOT IN list and nothing else touched. Copying from an
-- older definer would silently revert V602's mention exclusions and V596's
-- TrustLink ones. Before re-emitting this again, find the newest definer by
-- grepping for the definition, not by trusting a header: V597's own header
-- calls itself the definer and defines nothing.
--
-- ROLLBACK
--
-- Re-emit the procedure from V602 unchanged. Nothing else in this migration
-- has any state to undo. A table that stopped being copied is not deleted from
-- staging -- the sync leaves an excluded table exactly as it found it -- so
-- whatever staging holds today it keeps.
-- ============================================================================
