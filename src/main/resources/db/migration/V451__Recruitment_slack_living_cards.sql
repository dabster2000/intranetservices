-- ===================================================================
-- V451: Recruitment ATS expansion — Phase 22: Living candidate cards &
--       partner-track channels
-- ===================================================================
-- Feature: Recruitment ATS expansion (plan 2026-07-18 §P22,
--          Slack companion spec §4.1 / §5.1–5.2)
-- Domain:  recruitmentservice (Slack outbound projections)
--
-- Purpose:
--   1. recruitment_slack_threads — where each application's living root
--      card lives (channel + message ts), maintained by SlackCardReactor.
--      One row per application; the row's EXISTENCE is the idempotency
--      guard ("one root card per application" — a redelivery finds the
--      row and chat.updates instead of reposting). Holds NO candidate
--      data: uuids and Slack coordinates only.
--
--   2. recruitment_slack_channels — the private Slack channel of each
--      partner-track position, maintained by SlackChannelReactor
--      (create on POSITION_OPENED, archive on POSITION_CLOSED). Same
--      existence-as-idempotency pattern. No PII by schema.
--
--   3. Extend sp_sync_prod_to_staging's recruitment exclusion list with
--      the two new tables (the V450 MAINTENANCE rule: every new
--      recruitment_* table must be added, or the nightly sync copies it
--      from prod again). The tables hold no PII, but copying prod Slack
--      coordinates into staging would make staging reactors chat.update
--      PRODUCTION Slack messages — the exclusion is load-bearing either
--      way. Procedure body is V450 verbatim with only the two names
--      added.
--
--   No new app_settings rows: recruitment.slack.cards.enabled and
--   recruitment.slack.partner-channels.enabled were seeded OFF by V444.
--   The two reactors' offset rows need no seeding: the P1 startup guard
--   seeds every new reactor to the stream head at boot — no replay.
--
...
--   DDL is IF NOT EXISTS; the procedure DROP/CREATE is naturally
--   re-runnable.
--
-- Author: Claude Code
-- Date:   2026-07-24
-- Rollback: inert without the P22 images; everything is additive.
--   Full removal (only if the programme is abandoned):
--     DROP TABLE recruitment_slack_threads;
--     DROP TABLE recruitment_slack_channels;
--     DELETE FROM recruitment_reactor_offsets
--       WHERE reactor_name IN ('slack-cards', 'slack-partner-channels');
--     re-apply V450's procedure body.
-- ===================================================================

CREATE TABLE IF NOT EXISTS recruitment_slack_threads (
    application_uuid VARCHAR(36) NOT NULL PRIMARY KEY
        COMMENT 'Soft-FK recruitment_applications.uuid. Row exists = the application has a living root card.',
    channel_id VARCHAR(32) NOT NULL
        COMMENT 'Slack channel the root card was posted to (shared routed channel, or the partner-track private channel)',
    root_ts VARCHAR(32) NOT NULL
        COMMENT 'Slack message ts of the living root card — chat.update target and thread_ts for event replies',
    updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3)
        COMMENT 'UTC. Last time the reactor touched the card'
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci
  COMMENT = 'P22 living-card projection — one Slack root card per application; no PII by schema';

CREATE TABLE IF NOT EXISTS recruitment_slack_channels (
    position_uuid VARCHAR(36) NOT NULL PRIMARY KEY
        COMMENT 'Soft-FK recruitment_positions.uuid (hiring_track=PARTNER). Row exists = the private channel was created.',
    channel_id VARCHAR(32) NOT NULL
        COMMENT 'Slack channel id of the private recr-* channel (admin-token lifecycle)',
    archived_at DATETIME(3) NULL
        COMMENT 'UTC. Set when POSITION_CLOSED archived the channel; NULL while active'
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci
  COMMENT = 'P22 partner-track private-channel projection — circle membership mirrors recruitment_circle_members; no PII by schema';

-- -------------------------------------------------------------------
-- sp_sync_prod_to_staging: V450 body verbatim, with ONLY the two new
-- recruitment tables added to the cur_tables NOT IN (...) list.
-- -------------------------------------------------------------------
