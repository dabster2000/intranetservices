-- V466: Exclude the Phase 4 authorization tables from the prod-to-staging sync
-- (authorization-model-unification Phase 4, task 4.7 -- renumbered from V461).
--
-- sp_sync_prod_to_staging recreates every non-excluded table nightly with
-- FOREIGN_KEY_CHECKS = 0 (F-15). The four Phase 4 tables must survive it:
-- role_permission becomes UI-managed in Phase 7, and authz_version /
-- authz_audit are environment-local.
--
-- Established pattern (V258 api_clients, V453/V457 employee documents): the
-- FULL procedure body below is copied VERBATIM from V457 -- the latest
-- declaration -- with exactly one change: four names appended to the
-- TABLE_NAME NOT IN (...) exclusion list, marked with a V466 comment.
