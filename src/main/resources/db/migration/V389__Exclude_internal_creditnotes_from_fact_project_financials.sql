-- =============================================================================
-- V389: Exclude internal intercompany credit notes from fact_project_financials
--
-- fact_project_financials.total_revenue_dkk sums invoice lines with credit notes
-- signed -1 (invoice_line_companies CTE), admitting documents by type only
-- (INVOICE / PHANTOM / CREDIT_NOTE). The internal INTERNAL invoice is excluded by
-- that type filter, but an intercompany reversal is stored as type=CREDIT_NOTE with
-- a non-null debtor_companyuuid, so it slipped through and was subtracted one-sidedly
-- -- understating external project revenue / gross margin. This guard keeps only
-- external documents (external INVOICE/PHANTOM/CREDIT_NOTE all have
-- debtor_companyuuid IS NULL), mirroring Invoice.isInternalCreditNote(). Only the
-- invoice_line_companies WHERE clause changes; every other line is byte-identical to
-- the V214 definition.
--
-- NOTE: fact_project_financials feeds fact_project_financials_mat, which is rebuilt by
-- sp_refresh_fact_tables. Re-run sp_refresh_fact_tables after this migration so the
-- materialized table drops the historical understatement.
-- =============================================================================
