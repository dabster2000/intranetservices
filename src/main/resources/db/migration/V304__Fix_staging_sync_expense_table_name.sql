-- =============================================================================
-- Migration V304: Fix staging-sync expense-anonymisation typo + loud safeguard
--
-- BACKGROUND (see docs/superpowers/plans/2026-04-24-expenses-journal15-recovery.md)
--   V258 referenced `twservices4-staging`.`expense` (singular) in the expense
--   status-flip UPDATEs, but the actual table is `expenses` (plural). Those
--   UPDATEs raised "table doesn't exist", aborting the whole procedure. As a
--   result, VALIDATED production expenses got copied to staging with their
--   status intact for 49 nights, and the staging backend uploaded them to
--   e-conomics (journals 15/11/12) — poisoning production's idempotency keys.
--   117 expenses ended up UP_FAILED with errorCode=URLChanged.
--
-- CHANGES from V258:
--   1. `twservices4-staging`.`expense`  →  `twservices4-staging`.`expenses`
--      on the two UPDATE statements (the bug fix).
--   2. SIGNAL SQLSTATE '45000' post-condition check immediately after the
--      status-flip UPDATEs — if any upload-eligible status (VALIDATED,
--      PROCESSING, UP_FAILED, VOUCHER_CREATED) remains in staging.expenses
--      after the flip, raise a loud error so the MariaDB event logs the
--      abort and the CloudWatch RDS-event alarm (Prevention Layer 4) fires.
--
-- ROLLBACK:
--   Re-run V258 procedure body. (Note: this reintroduces the typo. Only do
--   this if the fixed procedure itself is broken.)
-- =============================================================================
