-- ============================================================================
-- V565: Exclude autofix_model_catalog from the prod -> staging sync
-- ============================================================================
-- Purpose: V564 adds autofix_model_catalog. Its siblings autofix_config and
--          autofix_tasks are already on the exclusion list; this table belongs
--          there for the same reason plus one of its own.
--
--          sp_sync_prod_to_staging DROPS AND RECREATES each non-excluded table
--          in staging from the PROD schema. For this table that would mean:
--
--          1. Any staging-only row an engineer added to trial a new model id
--             is silently reverted on the next nightly run — which defeats the
--             entire point of a table you edit by hand to test a model.
--          2. During a promotion window (staging on V564, prod not yet) a
--             column added on staging would be stripped back to prod's shape.
--
-- MySQL cannot patch a stored procedure, so the whole body is re-emitted. This
-- is a VERBATIM copy of the definition from V515 (the current one — V525 only
-- references it in a comment) with a single line added to the NOT IN list.
-- If you re-emit this proc again, copy from the newest migration that defines
