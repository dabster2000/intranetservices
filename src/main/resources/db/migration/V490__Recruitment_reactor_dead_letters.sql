-- ===================================================================
-- V490: Recruitment reactor dead letters — a poison-skipped event stops
--       vanishing into a log line
-- ===================================================================
-- Feature: Recruitment ATS event backbone (V433) — poison-event policy
-- Domain:  recruitmentservice.events
--
-- WHY
--   A reactor that overrides maxDeliveryAttempts() (SlackCardReactor and
--   SlackChannelReactor: 3) advances its watermark past an event that
--   failed every attempt. Until now the only trace was:
--
--     ERROR RecruitmentReactor - Reactor slack-cards: event seq 90 failed
--           3 attempts — skipping (poison event)
--
--   RecruitmentReactor.markSkipped() did write a 'SKIPPED' row into
--   recruitment_reactor_deliveries, but advanceWatermarkTo() — which runs
--   on the very next line for the same seq — prunes
--   `WHERE event_seq <= last_processed_seq`, deleting the marker it had
--   just written. So the durable SKIPPED marker promised by the
--   RecruitmentReactor javadoc never actually survived a sweep, and a
--   discarded event was unrecoverable and un-alarmed.
--
--   On 2026-08-11 that permanently dropped three production recruitment
--   Slack cards (seq 90, 91, 95) after chat.postMessage answered
--   channel_not_found: the partner-track private channel was created with
--   the ADMIN bot token while cards post with the MOTHER bot token, and a
--   private channel is invisible to a non-member bot.
--
-- WHAT
--   recruitment_reactor_dead_letters — one row per (reactor, event) that
--   exhausted its attempts. Written by markSkipped() in its own committed
--   transaction and NEVER pruned by the watermark, so the row outlives the
--   sweep that created it. RecruitmentEventCatchupBatchlet emits the
--   RECRUITMENT_REACTOR_DEAD_LETTER token while any row is OPEN, which
--   feeds the CloudWatch alarm created by
--   scripts/setup-recruitment-reactor-alarms.sh.
--
--   This mirrors the invoice_economics_uploads ABANDONED precedent (V478):
--   a terminal row a human resolves, not a counter that decays into
--   silence.
--
-- Resolution paths (RecruitmentReactorOpsResource, recruitment:admin):
--   GET    /recruitment/reactors/dead-letters              — list OPEN rows
--   POST   /recruitment/reactors/dead-letters/{reactor}/{seq}/replay
--          — re-run the handler now that the cause is fixed; REPLAYED on
--            success, stays OPEN with a refreshed error otherwise
--   POST   /recruitment/reactors/dead-letters/{reactor}/{seq}/abandon
--          — close a row whose side effect is moot or was done by hand
--
-- Collation: utf8mb4_general_ci, matching V433's three sibling tables.
--
...
-- Author: Claude Code
-- Date:   2026-08-12
-- Rollback: inert without the backend image that writes it. Full removal:
--     DROP TABLE recruitment_reactor_dead_letters;
--   (and re-apply V466's sp_sync_prod_to_staging to drop the exclusion).
-- ===================================================================

-- -------------------------------------------------------------------
-- 1. The dead-letter table
-- -------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS recruitment_reactor_dead_letters (
    reactor_name VARCHAR(100) NOT NULL
        COMMENT 'RecruitmentReactor.name()',
    event_seq BIGINT UNSIGNED NOT NULL
        COMMENT 'recruitment_events.seq that was skipped. Soft FK — the stream is append-only, so the row always resolves.',

    attempts INT NOT NULL DEFAULT 0
        COMMENT 'Delivery attempts made before giving up (maxDeliveryAttempts()).',

    error_class VARCHAR(255) NULL
        COMMENT 'Root-cause exception class, unwrapped past the JTA/transaction wrappers.',
    error_message VARCHAR(1000) NULL
        COMMENT 'Root-cause message, truncated. Reactor-facing diagnostics only — never a place to put candidate PII.',

    status VARCHAR(12) NOT NULL DEFAULT 'OPEN'
        COMMENT 'OPEN = needs a human. REPLAYED = re-delivered successfully. ABANDONED = deliberately closed unresolved.',

    dead_lettered_at DATETIME(3) NOT NULL
        COMMENT 'UTC. When the event exhausted its attempts.',
    last_attempt_at DATETIME(3) NULL
        COMMENT 'UTC. Refreshed by each replay attempt.',
    resolved_at DATETIME(3) NULL
        COMMENT 'UTC. Set when status leaves OPEN.',
    resolved_by VARCHAR(36) NULL
        COMMENT 'Soft-FK users.uuid of the operator who replayed/abandoned it.',

    PRIMARY KEY (reactor_name, event_seq),
    CONSTRAINT chk_rrdl_status_enum
        CHECK (status IN ('OPEN','REPLAYED','ABANDONED'))

) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_general_ci
  COMMENT='Recruitment ATS: events a reactor permanently skipped; survives the watermark, resolved by an operator';

-- The alarm and the ops list both read "everything still OPEN, oldest
-- first" every 5 minutes — keep it off a full scan.
CREATE INDEX IF NOT EXISTS idx_rrdl_status_seq
    ON recruitment_reactor_dead_letters (status, dead_lettered_at);

-- -------------------------------------------------------------------
-- 2. Extend the prod→staging sync exclusion list.
--    Established pattern (V258, V453/V457, V466): the FULL procedure body
--    below is copied VERBATIM from V466 — the latest declaration — with
--    exactly one change: 'recruitment_reactor_dead_letters' appended to
--    the TABLE_NAME NOT IN (...) list beside its V433 siblings, marked
--    with a V490 comment.
--
--    It belongs on the list for the same reason the rest of the block
--    does: error_message carries whatever a reactor's exception said, and
--    a dead letter references a prod seq that means nothing in staging —
--    replaying one there would target the wrong event entirely.
-- -------------------------------------------------------------------
