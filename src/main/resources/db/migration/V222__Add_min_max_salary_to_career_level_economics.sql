-- =============================================================================
-- Migration V222: Add min/max monthly salary to career level economics
--
-- Purpose:
--   Exposes per-career-level salary range (min/max) so the frontend can render
--   a salary range bar on the career level flow diagram cards.
--
-- Changes:
--   1. ALTER fact_minimum_viable_rate_mat — add min_monthly_salary_dkk and
--      max_monthly_salary_dkk columns (INT, nullable).
--   2. CREATE OR REPLACE VIEW fact_minimum_viable_rate — add MIN/MAX to the
--      avg_salary CTE and expose two new columns in the final SELECT.
--   3. Recreate sp_refresh_fact_tables — include the two new columns in block 11
--      (fact_minimum_viable_rate_mat refresh).
--
-- Backwards compatibility:
--   All existing columns are preserved. Two new columns are additive.
--   The SP is a full replacement (same as V221) with only block 11 and the
--   avg_salary CTE modified to include min/max salary.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. ALTER TABLE: add min/max salary columns to the materialized table
-- ---------------------------------------------------------------------------
ALTER TABLE fact_minimum_viable_rate_mat
    ADD COLUMN min_monthly_salary_dkk INT NULL AFTER avg_monthly_salary_dkk,
    ADD COLUMN max_monthly_salary_dkk INT NULL AFTER min_monthly_salary_dkk;


-- ---------------------------------------------------------------------------
-- 2. CREATE OR REPLACE VIEW: add MIN/MAX to avg_salary CTE
-- ---------------------------------------------------------------------------
...
WITH
    ...
    -- 3) Average/min/max monthly salary per company x career_level (current salary)
    avg_salary AS (
        SELECT
            us.companyuuid AS company_id,
            ccl.career_level,
...
SELECT
    ...
    -- Break-even utilization ratios (V207)
...
-- ---------------------------------------------------------------------------
-- 3. Recreate sp_refresh_fact_tables with min/max salary in block 11
--    Base: V221 (optimized SP). Only the avg_salary CTE and INSERT column
--    list in block 11 are changed.
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    --     V222: added min_monthly_salary_dkk and max_monthly_salary_dkk
    -- =========================================================================
