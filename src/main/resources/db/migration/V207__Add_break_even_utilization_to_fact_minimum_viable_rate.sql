-- =============================================================================
-- V207: Add break-even utilization columns to fact_minimum_viable_rate
--
-- Purpose:
--   Extends the fact_minimum_viable_rate view with three new computed columns
--   that express the minimum utilization rate required for a career level to
--   break even at 0%, 15%, and 20% margin targets.
--
-- Formula:
--   Break-Even Utilization = Total Monthly Cost / (Net Available Hours x Avg Billing Rate)
--   At margin X%:           Total Monthly Cost / ((1 - X%) x Net Available Hours x Avg Billing Rate)
--
-- New columns (appended after existing 26 columns):
--   break_even_utilization_pct      — break-even at 0% margin (pure cost recovery)
--   break_even_utilization_15pct    — break-even at 15% margin (divisor: 0.85)
--   break_even_utilization_20pct    — break-even at 20% margin (divisor: 0.80)
--
-- NULL handling:
--   Returns NULL when avg_net_available_hours is 0/NULL or avg_billing_rate is
--   0/NULL (i.e., no data to compute a meaningful ratio). Never returns 0 for
--   a missing denominator.
--
-- Precision:
--   ROUND(..., 4) — 4 decimal places, consistent with existing ratio columns
--   (e.g., actual_utilization_ratio, employer_pension_pct).
--
-- Backwards compatibility:
--   CREATE OR REPLACE VIEW — all 26 pre-existing columns are reproduced with
--   identical names, types, and positions. Only three new columns are appended
--   at the end. No existing queries referencing the view by column position or
--   name are broken.
--
-- Column references in expressions:
--   total_monthly_cost_dkk  — inline expression: asl.avg_monthly_salary + statutory
--                             + benefit + staff + overhead (same as existing column)
--   avg_net_available_hours — us2.avg_net_available_hours  (utilization_stats CTE, alias us2)
--   avg_actual_billing_rate — ar.avg_billing_rate           (actual_rates CTE, alias ar)
--
-- Source view definition: V178__Update_fact_minimum_viable_rate_career_level.sql
-- This migration is a full replacement (CREATE OR REPLACE) of that definition.
--
-- Affected Quarkus entities/repositories:
--   - Any Panache query on fact_minimum_viable_rate gains three new readable
--     columns — no existing queries break (additive change only).
--   - New BreakEvenUtilizationUseCase will SELECT these columns explicitly.
--
-- Rollback strategy:
--   Re-run V178 view definition to restore the original 26-column view.
--   The three new columns simply disappear; no data is lost (view, not table).
-- =============================================================================
...
SELECT
    ...
    -- -------------------------------------------------------------------------
    -- NEW COLUMNS (V207): Break-even utilization ratios
    --
    -- These answer: "What % utilization is required to cover all costs?"
    -- Formula: total_monthly_cost / (net_available_hours x avg_billing_rate)
    --
    -- NULL when avg_net_available_hours is 0/NULL or avg_billing_rate is 0/NULL.
    -- The COALESCE(us2.avg_net_available_hours, 0) guard ensures 0 triggers NULL.
    -- -------------------------------------------------------------------------

    -- Break-even utilization at 0% margin (pure cost recovery)
    ...
    -- Break-even utilization at 15% margin target (divisor: 0.85)
    CASE WHEN COALESCE(us2.avg_net_available_hours, 0) > 0 AND COALESCE(ar.avg_billing_rate, 0) > 0
        THEN ROUND(
            (asl.avg_monthly_salary
             + COALESCE(sr.pension_pct, 0) * asl.avg_monthly_salary
    ...
    -- Break-even utilization at 20% margin target (divisor: 0.80)
    CASE WHEN COALESCE(us2.avg_net_available_hours, 0) > 0 AND COALESCE(ar.avg_billing_rate, 0) > 0
        THEN ROUND(
            (asl.avg_monthly_salary
             + COALESCE(sr.pension_pct, 0) * asl.avg_monthly_salary
