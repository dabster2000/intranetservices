-- ============================================================================
-- V333: Fix fact_employee_monthly FTE basis and remove PROBATION dead code
-- ============================================================================
-- Purpose
--   Two corrections to the FTE / billable-classification logic that lives
--   duplicated in both `fact_employee_monthly` (the view) and
--   `sp_refresh_fact_tables` block 4 (the materialized table populator):
--
--     1. FTE divisor changes from 8.0 to 7.4. The Trustworks full-time
--        consultant day is 7.4 hours, not the generic 8.0 hours that the
--        formula was originally calibrated for. The view's daily_fte CTE
--        column was producing systematically low FTE numbers (FTE_actual
--        = FTE_reported * 8.0 / 7.4 ≈ 1.081 × reported). All downstream
--        consumers of `fte_total`, `fte_billable`, `fte_non_billable` will
--        rise by exactly +8.108% in lockstep at the next BI refresh.
--
--     2. The BILLABLE classification predicate
--        `b.status_type IN ('ACTIVE', 'PROBATION')` is reduced to
--        `b.status_type = 'ACTIVE'`. The userstatus enum has never had a
--        PROBATION value (verified: 0 rows in production), so the second
--        branch is dead code. Removing it eliminates a misleading hint for
--        future contributors and shortens the predicate.
--
--   Both corrections must land together in the same migration: the view
--   and the materialized-table CTE share an identical body. If only one
--   side were fixed, the view (`fact_employee_monthly`) and the table
--   (`fact_employee_monthly_mat`) would compute different FTE numbers
--   for the same month — a drift that would surface as inconsistencies
--   across executive dashboards, finance dashboards, and CXO reports.
--
-- Affected objects
--   - VIEW fact_employee_monthly             (recreated; CTE updated)
--   - PROCEDURE sp_refresh_fact_tables        (recreated; block 4 updated)
--
-- Scope of body changes
--   View `fact_employee_monthly`:
--     - daily_employee_data CTE:
--         BEFORE: AND b.status_type IN ('ACTIVE', 'PROBATION')
--         AFTER:  AND b.status_type IN ('ACTIVE')
--         BEFORE: COALESCE(b.gross_available_hours, 0) / 8.0 AS daily_fte
--         AFTER:  COALESCE(b.gross_available_hours, 0) / 7.4 AS daily_fte
--   Procedure `sp_refresh_fact_tables`:
--     - Block 4 (`fact_employee_monthly_mat`) daily_employee_data CTE:
--         identical two edits, mirroring the view.
--   All other 12 blocks of the procedure are unchanged byte-for-byte from
--   the V330 body.
--
-- Background
--   - The view was last touched in V330 (atomic rename to read from
--     fact_user_day instead of bi_data_per_day; behaviour preserved).
--   - The procedure was last touched in V330 (same rename for block 4).
--   - V310 introduced the optimised CTE form for the view (eliminated
--     correlated subqueries); the body is the source of truth.
--   - The 8.0 divisor and the PROBATION predicate predate V310 and were
--     copied forward unchanged across V275, V310, V330. This migration
--     is the first behavioural correction to either field.
--
-- Idempotency
--   DROP ... IF EXISTS guards both objects so the migration is re-runnable
--   in dev. In production it runs exactly once.
--
-- Rollback
--   Re-apply V330 verbatim (the view body and the procedure body are
--   complete in that file). Both objects are recreated in their entirety
--   in this migration, so a rollback is a single-shot replay of V330.
--
-- Verification (run after deploy)
--   -- 1. View FTE matches /7.4 convention. For a single ACTIVE billable
--   --    consultant on 2026-05-08:
--   SELECT useruuid, gross_available_hours,
--          ROUND(gross_available_hours / 7.4, 4) AS expected_fte
--   FROM fact_user_day
--   WHERE document_date = '2026-05-08'
--     AND status_type = 'ACTIVE'
--     AND consultant_type = 'CONSULTANT'
--   LIMIT 5;
--
--   -- 2. fact_employee_monthly_mat FTE for May 2026 should be ~+8.1%
--   --    higher than before deploy.
--   CALL sp_refresh_fact_tables();
--   SELECT company_id, practice_id, role_type, fte_total, fte_billable
--   FROM fact_employee_monthly_mat
--   WHERE year = 2026 AND month_number = 5
--   ORDER BY company_id, practice_id, role_type;
--
--   -- 3. PROBATION dead code is gone -- no row should have
--   --    billable_headcount = 0 alongside fte_billable > 0.
--   SELECT COUNT(*) FROM fact_employee_monthly_mat
--   WHERE billable_headcount = 0 AND fte_billable > 0;
--   -- expected: 0
--
--   -- 4. View and materialized table agree on FTE for current month.
--   SELECT
--       v.company_id, v.practice_id, v.role_type,
--       v.fte_total AS view_fte_total, m.fte_total AS mat_fte_total,
--       v.fte_billable AS view_fte_billable, m.fte_billable AS mat_fte_billable
--   FROM fact_employee_monthly v
--   JOIN fact_employee_monthly_mat m
--     ON v.company_id = m.company_id
--    AND v.practice_id = m.practice_id
--    AND v.role_type = m.role_type
--    AND v.month_key = m.month_key
--   WHERE v.year = 2026 AND v.month_number = 5
--     AND (ABS(v.fte_total - m.fte_total) > 0.01
--      OR  ABS(v.fte_billable - m.fte_billable) > 0.01);
--   -- expected: 0 rows (view and mat agree to 2 decimal places)
-- ============================================================================


-- ----------------------------------------------------------------------------
-- 1. fact_employee_monthly  -- /7.4 divisor; ACTIVE-only billable predicate
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2. sp_refresh_fact_tables  -- block 4 mirrors the view edits
--
-- The procedure body is V330's body verbatim with two surgical edits in
-- block 4 (the `daily_employee_data` CTE):
--   - daily_fte divisor 8.0 -> 7.4
--   - BILLABLE predicate IN ('ACTIVE', 'PROBATION') -> IN ('ACTIVE')
-- All other 12 blocks (1-3, 5-13) are byte-identical to V330's body.
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    -- V333: daily_fte divisor 8.0 -> 7.4 (Trustworks full-time day);
    --       billable predicate IN ('ACTIVE', 'PROBATION') -> IN ('ACTIVE')
    --       (PROBATION never appears in userstatus, dead code removed).
    -- =========================================================================
