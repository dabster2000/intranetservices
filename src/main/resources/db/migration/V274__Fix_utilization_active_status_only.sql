-- V274: Filter utilization calculation to ACTIVE status only
--
-- The utilization_by_user_month CTE included all non-TERMINATED statuses
-- (PREBOARDING, NON_PAY_LEAVE, MATERNITY_LEAVE, etc.), which have available
-- hours but zero billable hours — pulling utilization down ~10%.
--
-- Fix: Change status_type filter from <> 'TERMINATED' to = 'ACTIVE' so only
-- genuinely active consultant months count toward utilization.
