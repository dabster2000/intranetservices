-- ============================================================================
-- V594: Account Slack spaces — the linked channel's day, digested onto the timeline
-- ============================================================================
-- Feature: CRM spec §3.2 (the SLACK activity source), §4.9 and §6 (account spaces
--          linked to the account, a daily AI summary of decisions and next steps).
-- Domain:  aggregates/crm/slack + aggregates/crm/account
--
-- WHAT THIS IS
--   client_account.slack_space (V585) has held a channel name since the account
--   layer shipped, and exactly one thing ever read it: the DTO serializer. The
--   timeline's SLACK lane said "not connected". This migration gives the lane a
--   source: a nightly job reads the linked channel with the Slack API (a scheduled
--   OUTBOUND read — no inbound signing secret is involved, which is what kept
--   this unbuilt), asks a model for the day's decisions, next steps, risks and
--   client asks, and stores ONE row per (client, day).
--
-- THERE IS NO MESSAGE TEXT COLUMN, AND THAT IS THE POINT
--   Mirrors V588 ("there is no subject column"). What colleagues write in a
--   channel about a client names third parties, quotes e-mails, pastes links to
--   documents. None of it is stored. The message text exists in memory for the
--   length of one model call and is gone. What lands here is the model's
--   STRUCTURED reading — headline, decisions, next steps, risks, client asks,
--   client-side people named, topics — after the backend has re-validated,
--   capped and stripped it (AccountSlackDigestService). That reading is a
--   paraphrase of a conversation and still third-party-adjacent, so the two
--   tables join the staging-sync exclusion list below.
--
-- WHY ONE ROW PER DAY AND NOT ONE PER MESSAGE
--   The timeline is a feed a partner skims. Forty rows saying "Mikkel posted" is
--   noise; "#a_e-nettet: reelle ejere skubbes til efter kick-off; arkitektur-
--   tegning torsdag" is the day. The spec asked for a daily digest (§4.9).
--
-- WHY THE CHANNEL IS RESOLVED BY THE JOB, NOT ON SAVE
--   slack_space is a name typed by a person. A typo, a rename or a channel the
--   bot has not been invited to must not block SAVING the field, but must not
--   fail silently either — silent failure is exactly what the lane had before.
--   So the job resolves name → id (slack_channel_id) and records why it could
--   not (slack_link_error), and the header shows that next to the field.
--
-- WHY participants IS A TABLE
--   V593: "a set is a table in this schema, never a delimited string". Who
--   talked about the client on a given day is a set of employees with a count
--   each; the relationship graph can join it, a LIKE over a packed column could
--   not.
--
-- STAGING SYNC
--   account_slack_digest and account_slack_digest_participant join account_signal,
--   account_meeting and client_note on the sp_sync_prod_to_staging exclusion
--   list below. MySQL cannot patch a stored procedure, so the whole body is
--   re-emitted: this is a VERBATIM copy of the definition from V593 — the NEWEST
--   one — with two names added to the NOT IN list. If you re-emit this proc
--   again, copy from the newest migration that defines it, never from an older
--   one, or you silently revert later exclusions.
--
--   Note for staging: the sync nulls user.slackusername, so in staging the
--   participant mapping resolves nobody. Digests still form (counts + the model
--   reading); only the "who" is empty there.
--
-- RETENTION — THE OPEN GAP, EXTENDED
--   Spec §3.4/§3.7 promise 24 months after the account's last activity. The
--   purge job is still NOT built (V588, V590, V593 record the same gap). This
--   migration INCREASES the third-party-adjacent data at rest by one more table
--   pair; when the purge is built it must sweep account_slack_digest too. The
--   FK ON DELETE CASCADE means deleting a digest takes its participants with it.
--
-- FEATURE FLAG
--   crm.slack.account-spaces.enabled, seeded 'false'. Every Slack feature in this
--   repo is opt-in (V444's posture). Flag off ⇒ the job is a no-op and the page
--   is exactly as it was.
--
-- NO FOREIGN KEYS onto client / user, matching V585-V593. Inside the feature
--   account_slack_digest_participant → account_slack_digest IS a real FK.
--
-- RESERVED-WORD CHECK (MariaDB 10.x): none of digest_date, channel_id,
--   channel_name, message_count, thread_reply_count, last_message_ts, permalink,
--   relevance, headline, digest_json, model, prompt_version, synced_at,
--   slack_channel_id, slack_link_error, slack_synced_at is reserved (cf. the
--   V534 `lines` incident).
--
-- COLLATION: utf8mb4_general_ci, as V585-V593 (client_uuid / user_uuid join
--   general_ci tables; a unicode_ci mix fails at runtime with ERROR 1267).
--
-- Idempotency: ADD COLUMN IF NOT EXISTS, CREATE TABLE IF NOT EXISTS, INSERT ...
--   ON DUPLICATE KEY UPDATE.
--
-- Author: Claude Code
-- Date:   2026-09-13
-- Rollback:
--   DROP TABLE account_slack_digest_participant, account_slack_digest;
--   ALTER TABLE client_account DROP COLUMN slack_channel_id,
--                              DROP COLUMN slack_link_error,
--                              DROP COLUMN slack_synced_at;
--   DELETE FROM app_settings WHERE setting_key = 'crm.slack.account-spaces.enabled';
--   Re-emit sp_sync_prod_to_staging from V593 to drop the two exclusions.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. The resolved channel and why it could not be resolved
-- ----------------------------------------------------------------------------
ALTER TABLE client_account
    ADD COLUMN IF NOT EXISTS slack_channel_id VARCHAR(32) NULL
        COMMENT 'Resolved from slack_space by the nightly job; the Slack channel id (C…). Null until resolved, reset when the name changes',
    ADD COLUMN IF NOT EXISTS slack_link_error VARCHAR(40) NULL
        COMMENT 'Why the last resolution or read failed: NOT_FOUND | NOT_IN_CHANNEL | ARCHIVED; null when healthy',
    ADD COLUMN IF NOT EXISTS slack_synced_at DATETIME NULL
        COMMENT 'Last successful read of the channel';

-- ----------------------------------------------------------------------------
-- 2. One day of a linked channel. NO MESSAGE TEXT COLUMN EXISTS — see header.
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS account_slack_digest
(
    uuid               CHAR(36)     NOT NULL COMMENT 'Deterministic from (client_uuid, digest_date) so a re-sync updates',
    client_uuid        CHAR(36)     NOT NULL,
    channel_id         VARCHAR(32)  NOT NULL,
    channel_name       VARCHAR(80)  NOT NULL COMMENT 'Denormalised for the summary line; a rename does not rewrite history',
    digest_date        DATE         NOT NULL COMMENT 'Copenhagen calendar day',
    message_count      INT          NOT NULL COMMENT 'Human top-level messages that day',
    thread_reply_count INT          NOT NULL DEFAULT 0 COMMENT 'Human thread replies that day',
    last_message_ts    VARCHAR(32)  NOT NULL COMMENT 'Slack ts of the newest message read — the incremental cursor, never rendered',
    permalink          VARCHAR(500) NULL COMMENT 'Deep link to the newest message of the day',
    relevance          VARCHAR(8)   NULL COMMENT 'NONE | LOW | HIGH — the model''s own reading; null when the model was off or failed',
    headline           VARCHAR(200) NULL COMMENT 'The model''s one line for the timeline; null ⇒ the row composes a counts line at read time',
    digest_json        TEXT         NULL COMMENT 'The VALIDATED structured reading (SlackDigestContent). A paraphrase — never message text',
    model              VARCHAR(60)  NULL COMMENT 'Which model produced the reading, so a bad batch is attributable',
    prompt_version     VARCHAR(60)  NULL,
    synced_at          DATETIME     NOT NULL,
    PRIMARY KEY (uuid),
    UNIQUE KEY uq_account_slack_digest (client_uuid, digest_date),
    KEY idx_account_slack_digest_client (client_uuid, digest_date)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci
    COMMENT ='Per-day digest of a linked account Slack space (CRM spec 3.2). No message text column exists and none may be added.';

-- ----------------------------------------------------------------------------
-- 3. Who from Trustworks was talking that day
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS account_slack_digest_participant
(
    uuid          CHAR(36) NOT NULL,
    digest_uuid   CHAR(36) NOT NULL,
    user_uuid     CHAR(36) NOT NULL COMMENT 'Resolved from the Slack member id via user.slackusername; unresolvable ids are dropped',
    message_count INT      NOT NULL,
    PRIMARY KEY (uuid),
    UNIQUE KEY uq_account_slack_digest_participant (digest_uuid, user_uuid),
    KEY idx_account_slack_digest_participant_user (user_uuid),
    CONSTRAINT fk_account_slack_digest_participant_digest FOREIGN KEY (digest_uuid)
        REFERENCES account_slack_digest (uuid) ON DELETE CASCADE
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci
    COMMENT ='Trustworks people active in the account space on a digest day';

-- ----------------------------------------------------------------------------
-- 4. The switch. Off until somebody turns it on — every Slack feature is opt-in.
-- ----------------------------------------------------------------------------
INSERT INTO app_settings (setting_key, setting_value, category)
VALUES ('crm.slack.account-spaces.enabled', 'false', 'crm')
ON DUPLICATE KEY UPDATE setting_key = VALUES(setting_key);

-- ----------------------------------------------------------------------------
-- 5. Staging sync — the two digest tables must never be copied
--
--    VERBATIM re-emission of sp_sync_prod_to_staging as defined in V593 (the
--    newest definition), with two names added to the NOT IN list. See the
