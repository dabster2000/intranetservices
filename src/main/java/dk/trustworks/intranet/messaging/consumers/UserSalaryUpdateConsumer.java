...
public class UserSalaryUpdateConsumer {
    ...
    @WithSpan("consumer.user-salary-updates")
    @Blocking
    public CompletionStage<Void> onMessage(Message<String> msg) {
        ...
        try {
            ...
            errors.increment();
            sample.stop(timer);
            log.errorf(e, "UserSalaryUpdateConsumer failed channel=%s topic=%s partition=%d offset=%d key=%s", CHANNEL, topic, partition, offset, key);
            return msg.nack(e);
        }
    }
}
