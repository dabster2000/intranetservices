...
@JBossLog
@ApplicationScoped
public class WorkUpdateConsumer {

    private static final String CHANNEL = "work-updates";

    @Inject
    WorkAggregateService workAggregateService;

    @Inject
    dk.trustworks.intranet.bi.services.UserAvailabilityCalculatorService userAvailabilityCalculatorService;

    @Inject
    dk.trustworks.intranet.bi.services.BudgetCalculatingExecutor budgetCalculatingExecutor;
    ...
    @WithSpan("consumer.work-updates")
    @Blocking
    ...
    public CompletionStage<Void> onMessage(Message<String> msg) {
        ...
        try {
            ...
            // 1) Update availability for the day
            userAvailabilityCalculatorService.updateUserAvailabilityByDay(useruuid, date);
            // 2) Recalculate work aggregates for the day
            workAggregateService.recalculateWork(useruuid, date);
            // 3) Recalculate budgets for the day
            budgetCalculatingExecutor.recalculateUserDailyBudgets(useruuid, date);

            long durMs = (System.nanoTime() - startNs) / 1_000_000;
            log.infof("Processed work update (availability -> work -> budget) topic=%s partition=%d offset=%d key=%s user=%s date=%s durationMs=%d", topic, partition, offset, key, useruuid, date, durMs);
            success.increment();
            sample.stop(timer);
            return msg.ack();
        } catch (Exception e) {
            errors.increment();
            sample.stop(timer);
            log.errorf(e, "WorkUpdateConsumer failed channel=%s topic=%s partition=%d offset=%d key=%s", CHANNEL, topic, partition, offset, key);
            return msg.nack(e);
        }
    }
}
