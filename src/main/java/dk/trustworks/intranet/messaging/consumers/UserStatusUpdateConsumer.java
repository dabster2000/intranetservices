...
/**
 * Kafka consumer for user status update events.
 *
...
public class UserStatusUpdateConsumer {
    ...
    @WithSpan("consumer.user-status-updates")
    @Blocking
    public CompletionStage<Void> onMessage(Message<String> msg) {
        ...
        try {
            ...
            long durMs = (System.nanoTime() - startNs) / 1_000_000;
            log.infof("User status update noted (BI recalc via DB triggers) topic=%s partition=%d offset=%d key=%s user=%s date=%s durationMs=%d",
            ...
            errors.increment();
            sample.stop(timer);
            log.errorf(e, "UserStatusUpdateConsumer failed channel=%s topic=%s partition=%d offset=%d key=%s", CHANNEL, topic, partition, offset, key);
            return msg.nack(e);
        }
    }
}
