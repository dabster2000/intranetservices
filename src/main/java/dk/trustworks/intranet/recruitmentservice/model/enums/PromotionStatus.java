 ...
 * §6.5.3). No PARTIAL: promotion is idempotent per file
 * ({@code migrated_from} provenance), so a re-run simply skips files that
 * already have a row and completes the rest. NULL on the candidate row =
 * not converted (or converted before promotion existed).
 */
