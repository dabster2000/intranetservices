...
public class EconomicsCustomerPairingService {
    ...
    public PairingRowDto createAndPair(UUID clientUuid, UUID companyUuid) {
        ...
        EconomicsCustomerDto body = toCreateBody(c);
    ...
    private static EconomicsCustomerDto toCreateBody(Client c) {
        ...
        // e-conomic's auto-assign. Non-numeric CVR or missing CVR → null, and
        // e-conomic assigns one.
        if (c.getCvr() != null && !c.getCvr().isBlank()) {
            try {
                body.setCustomerNumber(Integer.parseInt(c.getCvr().trim()));
            } catch (NumberFormatException ignore) {
                // Non-numeric CVR: let e-conomic assign a number.
            }
        }
