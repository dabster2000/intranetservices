...
public class PhantomClientResolver {
    ...
    public PhantomClientSuggestion suggest(String clientname) {
        ...
        //    Returns the first client (DB order) whose name contains the label.
        //    A "found" suggestion must carry a real uuid, so a name match with a
        //    null uuid is skipped rather than returned as a candidate.
        String needle = stripped.toLowerCase(Locale.ROOT);
        for (Client c : clientService.listAllClients()) {
