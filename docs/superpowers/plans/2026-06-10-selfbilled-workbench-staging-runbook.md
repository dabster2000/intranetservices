...
Caveats: all staging mutations (assignments, QUEUED internals 53df09ce + 73c812fb, link stamp, code map, ignores) are wiped by the nightly prod→staging refresh (~02:00). The step-11 contract fix (`9aa89534`) is permanent code. Outstanding before prod: (1) ~~admin sets `invoice-product-number` for TECH + CYBER~~ DONE on prod 2026-06-10 (value `1` = Konsulenttimer, matching A/S; propagates to staging via the nightly refresh) — force-create re-verification deliberately NOT run (would post a real draft into live e-conomic; left for the accountant during prod rollout); (2) on prod, re-seed the code map via POST /code-map (MC→Michelle on 2104, JK→Julie on 2106) — the staging fix is wiped nightly and prod tables start empty.
